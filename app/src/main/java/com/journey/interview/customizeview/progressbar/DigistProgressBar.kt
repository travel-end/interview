package com.journey.interview.customizeview.progressbar
...
class DigistProgressBar @JvmOverloads constructor(
    context: Context,
     ...
     * Listener
     */
    private var mListener: OnDigistListener? = null
    ...
    init {
        ...
        val attributes = context.theme.obtainStyledAttributes(
            attrs, R.styleable.DigistProgressBar,
            defStyleAttr, 0
        )

        mReachedBarColor = attributes.getColor(
            R.styleable.DigistProgressBar_progress_reached_color,
            default_reached_color
        )
        mUnreachedBarColor = attributes.getColor(
            R.styleable.DigistProgressBar_progress_unreached_color,
            default_unreached_color
        )
        textColor = attributes.getColor(
            R.styleable.DigistProgressBar_progress_text_color,
            default_text_color
        )
        mTextSize = attributes.getDimension(
            R.styleable.DigistProgressBar_progress_text_size,
            default_text_size
        )

        reachedBarHeight = attributes.getDimension(
            R.styleable.DigistProgressBar_progress_reached_bar_height,
            default_reached_bar_height
        )
        unreachedBarHeight = attributes.getDimension(
            R.styleable.DigistProgressBar_progress_unreached_bar_height,
            default_unreached_bar_height
        )
        mOffset = attributes.getDimension(
            R.styleable.DigistProgressBar_progress_text_offset,
            default_progress_text_offset
        )

        val textVisible = attributes.getInt(
            R.styleable.DigistProgressBar_progress_text_visibility,
        ...
        progress = attributes.getInt(R.styleable.DigistProgressBar_progress_current, 0)
        max = attributes.getInt(R.styleable.DigistProgressBar_progress_max, 100)
    ...
    fun setOnProgressBarListener(listener: OnDigistListener) {
        mListener = listener
    }
