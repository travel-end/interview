 ...
 */
public class SpiderWebView2 extends View {
    // 控件宽高
    private int mWidth;
    private int mHeight;
    // 画笔
    ...
    // 数据源
    private List<SpiderPoint2> mSpiderPointList;
    // 相关参数配置
    private SpiderConfig2 mConfig;
    // 随机数
    private Random mRandom;
    // 手势帮助类 用于处理滚动与拖拽
    private GestureDetector mGestureDetector;

    public SpiderWebView2(Context context) {
        this(context, null);
    }

    public SpiderWebView2(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SpiderWebView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        // setLayerType(LAYER_TYPE_HARDWARE, null);

        mSpiderPointList = new ArrayList<>();
        mConfig = new SpiderConfig2();
        mRandom = new Random();
    ...
    private void initPoint() {
        for (int i = 0; i < mConfig.pointNum; i++) {
            ...
            SpiderPoint2 point = new SpiderPoint2(width, height);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 增强遍历
        int index = 0;
        for (SpiderPoint2 spiderPoint : mSpiderPointList) {
            ...
            for (int i = index; i < mSpiderPointList.size(); i++) {
                SpiderPoint2 point = mSpiderPointList.get(i);
                // 判定当前点与其他点之间的距离
